...
**Lo único despachable a partir de aquí es la línea backend, y hoy no lo está:** los siete RFC de `B` siguen en `propuesto`. Aprobar el de **B01** es el único paso que separa al proyecto de tener un sprint habilitado — no depende de nada y todas sus fuentes están aprobadas. Los demás `B` se encadenan detrás según la columna «Depende de».
